...
const INTERNAL_BOOTSTRAP_MESSAGE = '[RUFRONE_RATCHET_BOOTSTRAP_V1]';
...
export class HandshakeManager {
    ...
    canSendEncrypted() {
        return this.state === 'ESTABLISHED' && !!this.ratchet.sendChainKey;
    }
    ...
    async handleIncomingMessage(messageData) {
        ...
        if (packet.type === "HANDSHAKE_OFFER" && this.state === 'IDLE') {
            ...
            if (plaintext === INTERNAL_BOOTSTRAP_MESSAGE) return;
    ...
    async _handleAccept(payload) {
        ...
        this.state = 'ESTABLISHED';
        this.tempKeyPair = null;
        await this._sendBootstrapMessage();
        if (this.onHandshakeEstablished) this.onHandshakeEstablished();
    }
    ...
    async _sendBootstrapMessage() {
        if (!this.canSendEncrypted()) return;
        await this.sendEncryptedMessage(INTERNAL_BOOTSTRAP_MESSAGE);
    }
